...
DONOHUE_DF %>%
  ...
  scale_x_continuous(breaks = seq(1980, 2015, by = 5),
                     limits = c(1980, 2015),
                     labels = c(seq(1980, 2010, by = 5), rep("", 1))) +
...
DONOHUE_OUTPUT_TIDY$Analysis <- "Donohue, et al."
```
...
LOTT_OUTPUT_TIDY$Analysis <- "Lott and Mustard"
```
...
comparing_analyses_plot <- ggplot(comparing_analyses) + 
  ...
  geom_errorbar(aes(x = Analysis, ymin = conf.low, ymax = conf.high)) + 
  geom_hline(yintercept = 0, color = "red") +
  theme_minimal() + 
  labs(title = "Effect of right-to-carry law on ln(violent crime rate per 100,000)",
       y = "Point estimate (95% CI)")
...
DONOHUE_DF %>% 
  ...
  ggpairs(.,
          columns = c(2:4),
          lower = list(continuous = wrap("smooth_loess", alpha = 0.1, size = 0.1)))
...
```{r, fig.width=10, fig.height=7}
DONOHUE_DF %>%
  dplyr::select(contains("_years")) %>%
  ggcorr(method = c("pairwise.complete.obs", "pearson"),
         hjust = 0.5,
         vjust = 0,
         color = "transparent") +
  labs(title = "Pearson's Correlation Coefficient by Demographic Group")
```
...
```{r, eval=FALSE}
plot_grid(comparing_analyses_plot)
```
