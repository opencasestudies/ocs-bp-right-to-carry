...
First we need to check that our data is indeed ready to be joined. We need ot make sure that the colum names are the same for each dataset that we intend to combine together. 

can use setequal setdiff or all_equal...

We will use the `set_equal()` function of the `dplyr` package to compare these. This does not require that the order be the same. Similar functions like `set_diff()` and `all_equal()` however do require the same order, or at least have an argument to specify if you want to check for the same order.
setequal(X, Y)
...
colnames(poverty_rate_data)
colnames(crime_data)
colnames(ps_data) #police staffing
...
Avocado bind_rows will make na values for the later years beyond 2010 for the datasets that have them... not sure why we are adding these... maybe more about that later?


We will now bind the demogrpahic data that we made for the Donohue-like analysis called `dem_DONOHUE`, as well as all the other datasets that we have wrangled. This is possible becuase we have the same colnames for each dataset.
