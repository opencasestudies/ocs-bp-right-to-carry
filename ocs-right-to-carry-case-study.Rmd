...
Avocado... not sure what Michael meant by this: Need to fix initialization problem 


This case study will focus on one reason why two papers that explore the relationship of the adoption of right-to-carry (RTC) concealed handgun laws and violent crime show conflicting findings:

1) The first analysis by [Lott and Mustard](https://chicagounbound.uchicago.edu/cgi/viewcontent.cgi?article=1150&context=law_and_economics){target="_blank"} published in 1996 suggests that RTC laws reduce violent crime.

```{r, echo=FALSE, out.height = '100%', out.width = '100%', fig.align='center'}
knitr::include_graphics(here("img", "Lott.png"))
```

2) The second analysis by [Donohue, et al.](https://www.nber.org/papers/w23510.pdf){target="_blank"} published in 2017 suggests that RTC laws increase violent crime.

```{r, echo=FALSE, out.height = '100%', out.width = '100%', fig.align='center'}
knitr::include_graphics(here("img", "Donohue.png"))
```

These are by no means the only articles published about the question of the influence of RTC laws on violent crime. This has been a controversial topic as many other articles since the [Lott and Mustard](https://chicagounbound.uchicago.edu/cgi/viewcontent.cgi?article=1150&context=law_and_economics){target="_blank"} also had conflicting results. 

The [Donohue, et al.](https://www.nber.org/papers/w23510.pdf){target="_blank"} article discusses how there are many other important methodolical aspects that could account for the historically conflicting results in these previous papers.
...
We will focus particularly on one aspect that was very influential. It is something called multicollinearity and we will explore how it can influence linear regression analyses and result in different conclusions. 
...
Therefore we will use a subset of the listed explanatory variables and they will be consistent for both analyses that we will perform, however one analysis will have 6 demographic variables like the analysis in the [Donohue, et al.](https://www.nber.org/papers/w23510.pdf){target="_blank"} article and the other will have 36 demogrpahic variables like the analysis in the [Lott and Mustard](https://chicagounbound.uchicago.edu/cgi/viewcontent.cgi?article=1150&context=law_and_economics){target="_blank"}article.
...
It is a law that allows citizens **  not surea about this part** who obtain a permit to have a handgun on their person or nearby (for example in the citizen's car) in public if it is concealed. 

This is no federal law about carrying concealed guns. These laws are legistalted at the state level and some states have chosen to adopt such laws, while others do not allow citizens to legally carry a concealed handgun. 
...
However, there are no federal laws about the right to carry firearms in public. 
...
Each state has specific laws about the right to carry firearms with the following general categories:
