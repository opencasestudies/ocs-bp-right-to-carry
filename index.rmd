...
pooled: $$Y_{it}=β{0}+β{1}x_{1it}+...+β_KX_{Kit}$$
like this for fixed effect: $$Y_{it}=β_{0i}+β_{1i}X_{1it}+β_{2i}X_{2it}+e_it$$
...
To do so we will perform a process called [resampling](https://en.wikipedia.org/wiki/Resampling_(statistics)){target="_blank"}. This invovles peforming multiple iterations of our analysis, but with only a subset or subsample of our orignial data.  In our case we will remove **one** observation and see if that changes our coefficient estimate results. 
...
simulation_plot <- coeff_sd %>%
 ...
 geom_jitter(width = 0.1, alpha = 0.5, size = 2) + 
  labs(title = "Coefficient instability",
       subtitle = "Coefficient estimates may be more unstable",
       x = "Term",
       y = "Standard deviation of coefficient estimates",
       caption = "Results from simulations") + 
  theme_linedraw() +
  theme(axis.title.x = element_blank(),
...
Since VIF values are calculated using only the independent variables, we don't need to worry about the effect of time and indiviuals in our calculation, thus we will use the pooled model. Otherwise, the `vif()` function is incompatible with the output from the `plm()` function.
Then we will create nicer looking output of the data using the `as_tibble()`function of the `tibble` package to create a tibble and add the varaible names as another column.
...
Idk if we need to/should do this. The values are different.- lower in general.
...
vif_plot
```
We can see that both analyses have some variables with relatively high multicolinearity, however analysis 2 has variables with very high multicolinearity.
...
In any case the presence of multicolinearity should encourage further investigation about the design of the model, as the results may not be reliable due to the increased level of instability of the coeffecients estimates.
...
avocado  maybe add somewhere that pml drops perfectly colinear variables automatically.. they will not be in the output
...
For our second row in our larger plot we want to have the formula for cacluating VIF values on the left and the plot that we created previously showing VIF values on the right.

First we will create a plot object that just has the forumla.
...
To type the formla we need to use [LaTeX mathematical notation](https://www.calvin.edu/~rpruim/courses/s341/S17/from-class/MathinRmd.html){target="_blank"}.

The start and end of innline mathematical formulas are specified using dollar signs (`$`).   
...
Greek letters  are created by using `\alpha`
...
plot_B1 <- ggplot(empty_df, aes(x = X, y = Y)) +
   annotate("text",
           x=5,
           y=8,
           label = TeX("$X_{1} = \\alpha_{0} + \\alpha_{2}X_{2} + \\alpha_{3}X_{3}...+\\alpha_{k}X_{k}$"),
           size = 8)+ ylim(0,10)+xlim(0,10)+
   annotate("text",
           x=5.9,
           y=6.5,
           label = TeX("$R^{2}$"),
           size = 8)+ ylim(0,10)+xlim(0,10)+
geom_segment(aes(x = 5, y = 7.5, xend = 5, yend = 6),
               arrow = arrow(angle = 45, ends = "last", type = "open"),
               size = 2,
               color = "black",
               lineend = "butt",
...
This case study has introduced the concept of multicolinearity by exploring data related to violent crimes and right-to-carry gun laws. We also intrudced the topic of panel data as a special type of longitinal data that includes data of 2 or more individuals or groups over 2 or more time points. We learned that we can use the `plm` package to perform panel linear regression analysis. We learned that the fixed effect model in panel analysis actually makes the least assumptions, and is therefore often the most appropriate test.

By evaluating two analyses that were identical except for the inclusion of extra demographic variables, analysis 1 included 6, while analysis 2 included 36, we discovered that redudant and colinear variables can change the directionality and magnitude of our findings. 

We learned that by looking at the correlation between pairs of explanatory variabels we can get a sense about whether multicolinearity may exist in our data. We also learned that correaltion does not necessarily imply multicolinearity. Nor does multicolinearity imply correlation.

We learned that we can evaluate the stability of our coeffecient estimates across subsamples or cacluate variance inflation factor (VIF) values to get a sense of the presence and severity of multicolinearity.

We learned that often a rule of thumb of >10 is used as a threshold for raising concern about the severity of multicolinearity. However, we also learned that (as often is the case with thersholds) more care may be required. 

Overall we learned that multicolineartiy can bias our regression findings and it is good practice to check for multicolinearity when performing regession analysis.  It is something to keep in mind when we encounter coefficient estimates that are unexpected. 

Importantly this case study showcases how methodolgical details, like how we decide to parse our demographic variables can have great consequences on the results of our analyses. 


# **Suggested Homework**
*** 

Ask the students to remove one or more of the demographic variables with high VIF values from the LOTT- like panel data and perform the panel linear regression analysis again, as well as cacluate the VIF values. 
