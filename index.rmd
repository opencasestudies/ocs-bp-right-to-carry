...
To do so we will perform multiple iterations of our analysis, but we will remove one observation and see if that changes our coefficient estimate results. First we will make 250 different samples that are missing one observation.  Note that this causes our data to be an unbalanced panel.

This does not require any adjustment to the code to model the data, but you will notice that the output will now say "unbalanced".AVOCADO check this
...
https://cfss.uchicago.edu/notes/cross-validation/
...
pull(test_samples, splits)[[1]]

training(pull(test_samples, splits)[[1]])
training(pull(test_samples, splits)[[2]])
...
head(training(pull(DONOHUE_splits, splits)[[1]]))
...
#looks like the Texas data from 1988 was removed from the first split
d_panel_DONOHUE %>%
  filter(! rownames(d_panel_DONOHUE)  %in% rownames(DONOHUE_subsets[[1]]))

#where as Louisiana data from 1981 was removed in the second split
d_panel_DONOHUE %>%
  filter(! rownames(d_panel_DONOHUE)  %in% rownames(DONOHUE_subsets[[2]]))
